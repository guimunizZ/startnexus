...
export default function PublicAssistancePage() {
    ...
    useEffect(() => {
        async function loadData() {
            try {
                ...
                const finalReviews = reviewData || [];
                setReviews(finalReviews);

                if (finalReviews.length > 0) {
                    const total = finalReviews.reduce(
                        (sum, item) => sum + item.rating,
                        0
                    );

                    setAvgRating(total / finalReviews.length);
                }
            } catch (error) {
                console.error(error);
                router.push("/b2c");
    ...
    const services: ServiceItem[] =
        assistance.services_offered?.length > 0
            ? assistance.services_offered.map((item) => ({
                title: item,
                description:
        ...
        <div className="min-h-screen bg-slate-950 text-white">
            ...
            <main className="max-w-7xl mx-auto px-6 py-8 space-y-8">
                <PublicAssistanceHero
                    ...
                    name={assistance.business_name}
                    rating={avgRating}
                    reviews={reviews.length}
                    verified={true}
                    junior={assistance.is_junior}
              ...
              {services.length} serviços
                ...
                <section className="space-y-5">
                    ...
                    <div className="space-y-4">
                        {reviews.map((item) => {
                            const isMine = item.client_id === userId;
